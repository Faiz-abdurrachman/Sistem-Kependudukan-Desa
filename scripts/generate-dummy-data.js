...
function generateWilayahData() {
  const wilayah = [];
  const dusunList = ['Dusun Krajan', 'Dusun Sumber', 'Dusun Pasar', 'Dusun Kebon', 'Dusun Sawah'];
...
function generateKKData(wilayahData) {
  const kk = [];
  const alamatPrefix = ['Jl. Raya', 'Jl. Kampung', 'Jl. Sawah', 'Jl. Kebon', 'Jl. Pasar'];
  ...
  wilayahData.forEach((wilayah, index) => {
    kk.push({
      ...
      alamat_lengkap: `${alamatPrefix[index % alamatPrefix.length]} ${wilayah.dusun} RT ${wilayah.rt}/RW ${wilayah.rw}`,
...
function generatePendudukData(kkData) {
  const penduduk = [];
  const namaDepan = ['Ahmad', 'Siti', 'Budi', 'Dewi', 'Eko', 'Fitri', 'Gunawan', 'Hani', 'Indra', 'Joko', 'Kartika', 'Lukman', 'Maya', 'Nur', 'Omar', 'Putri', 'Rudi', 'Sari', 'Tono', 'Umi'];
  const namaBelakang = ['Santoso', 'Rahayu', 'Wijaya', 'Sari', 'Prasetyo', 'Lestari', 'Kurniawan', 'Sari', 'Hidayat', 'Sari', 'Wulandari', 'Setiawan', 'Indrawati', 'Hidayati', 'Purnomo', 'Sari', 'Wibowo', 'Sari', 'Saputra', 'Sari'];
  ...
  const shdk = ['KEPALA KELUARGA', 'SUAMI', 'ISTRI', 'ANAK', 'MENANTU', 'CUCU', 'ORANG TUA', 'MERTUA', 'FAMILI LAIN'];
  const pendidikan = ['TIDAK / BELUM SEKOLAH', 'BELUM TAMAT SD', 'SD / SEDERAJAT', 'SMP / SEDERAJAT', 'SMA / SEDERAJAT', 'D1 / D2 / D3', 'D4 / S1', 'S2', 'S3'];
  const pekerjaan = ['PNS', 'Wiraswasta', 'Petani', 'Buruh', 'Guru', 'Dokter', 'Perawat', 'Pedagang', 'Karyawan Swasta', 'Tidak Bekerja', 'Pelajar/Mahasiswa'];
  const statusDasar = ['HIDUP', 'HIDUP', 'HIDUP', 'HIDUP', 'HIDUP', 'MATI', 'PINDAH']; // 5:1:1 ratio
  ...
  kkData.forEach((kk, kkIndex) => {
    ...
    for (let i = 0; i < anggotaCount; i++) {
      let jenisKel = jenisKelamin[Math.floor(Math.random() * jenisKelamin.length)];
      ...
      const tglLahir = `${tahunLahir}-${String(bulanLahir).padStart(2, '0')}-${String(hariLahir).padStart(2, '0')}`;
      
      // Kepala keluarga biasanya laki-laki, sudah kawin, usia 30-60
      let shdkValue = shdk[Math.floor(Math.random() * shdk.length)];
      let statusKawinValue = statusKawin[Math.floor(Math.random() * statusKawin.length)];
      ...
      penduduk.push({
        ...
        status_dasar: statusDasar[Math.floor(Math.random() * statusDasar.length)],
        catatan: null
      });
    }
  });
...
function generateMutasiData(pendudukData) {
  const mutasi = [];
  const jenisMutasi = ['PINDAH MASUK', 'PINDAH KELUAR', 'LAHIR', 'MATI', 'PERUBAHAN STATUS'];
...
function generateSuratKeluarData(pendudukData) {
  const surat = [];
  const jenisSurat = ['SURAT KETERANGAN DOMISILI', 'SURAT KETERANGAN TIDAK MAMPU', 'SURAT KETERANGAN USAHA', 'SURAT KETERANGAN KELAKUAN BAIK', 'SURAT PENGANTAR KTP'];
  ...
  pendudukForSurat.forEach((penduduk, index) => {
    ...
    const nomorSurat = `SK/${String(index + 1).padStart(4, '0')}/${new Date(tanggalCetak).getFullYear()}`;
